...
function SparseProblemMOI(n_nlp,
    ...
    sparsity_jac = get_sparsity_pattern(temp_con_jac)
    sparsity_hess = get_sparsity_pattern(temp_lag_hess)
    obj_grad=true
    con_jac=true
    hessian_lagrangian=true
...
function MOI.eval_hessian_lagrangian(prob::SparseProblemMOI, H, x, σ, λ)

    prob.lagrangian_hessian!(prob.params, prob.temp_lag_hess, x, σ, λ)

    H .= prob.temp_lag_hess.nzval
    return nothing
end
...
MOI.hessian_lagrangian_structure(prob::SparseProblemMOI) = prob.sparsity_hess
